...
class MPM_Simulator_WARP:
    ...
    def initialize(self, n_particles, n_grid=100, grid_lim=1.0, device="cuda:0"):
        ...
        self.mpm_model.E = wp.zeros(shape=1, dtype=float, device=device, requires_grad=True)
        self.mpm_model.scale_E = 1e7   # 缩放尺度
        # add
        ...
        self.mpm_model.nu = wp.zeros(shape=1, dtype=float, device=device, requires_grad=True)
        ...
        self.mpm_model.yield_stress = wp.zeros(shape=1, dtype=float, device=device, requires_grad=True)
        self.mpm_model.scale_ys = 1e5   # yield_stress缩放尺度
        self.mpm_model.plastic_viscosity = wp.zeros(shape=1, dtype=float, device=device, requires_grad=True)
        self.mpm_model.scale_pv = 1e4   # plastic_viscosity缩放尺度
        self.mpm_model.xi = 0.0  # wp.zeros(shape=n_particles, dtype=float, device=device, requires_grad=True)
        self.mpm_model.softening = wp.zeros(shape=n_particles, dtype=float, device=device, requires_grad=True)
        self.mpm_model.friction_angle = wp.zeros(shape=1, dtype=float, device=device, requires_grad=True)
    ...
    def set_parameters_dict(self, kwargs={}, device="cuda:0"):
        if "material" in kwargs:
            if kwargs["material"] == "jelly" or kwargs["material"] == "elastic":
                self.mpm_model.material = 0
            ...
            elif kwargs["material"] == "newtonian":
                self.mpm_model.material = 5
            elif kwargs["material"] == "non_newtonian":
        ...
        if "scale_E" in kwargs:   # <<< add
            self.mpm_model.scale_E = kwargs["scale_E"]
        # add 
        if "mu_N" in kwargs:
            wp.launch(
                kernel=set_value_to_float_array,
        ...
        if "scale_ys" in kwargs:
            self.mpm_model.scale_ys = kwargs["scale_ys"]
        if "plastic_viscosity" in kwargs:
            val = kwargs["plastic_viscosity"]
            wp.launch(
                kernel=set_value_to_float_array,
                dim=1,
                inputs=[self.mpm_model.plastic_viscosity, val],
                device=device,
            )
        if "scale_pv" in kwargs:   # <<< add
            self.mpm_model.scale_pv = kwargs["scale_pv"]
        if "hardening" in kwargs:
            self.mpm_model.hardening = kwargs["hardening"]
            if self.mpm_model.material == 1 or self.mpm_model.material == 4: # metal or plasticine
                self.mpm_model.hardening = 1
        if "xi" in kwargs:
            self.mpm_model.xi = kwargs["xi"]
        if "friction_angle" in kwargs:
            val = kwargs["friction_angle"]
            wp.launch(
                kernel=set_value_to_float_array,
                dim=1,
                inputs=[self.mpm_model.friction_angle, val],
                device=device,
            )
        ...
        # finalize per-particle mu/lam from global E/nu and print diagnostics
        try:
            self.finalize_mu_lam(device=device)
            # gather stats via torch tensors
            E_t = wp.to_torch(self.mpm_model.E)
            mu_t = wp.to_torch(self.mpm_model.mu)
            lam_t = wp.to_torch(self.mpm_model.lam)
            yield_t = wp.to_torch(self.mpm_model.yield_stress)
            pv_t = wp.to_torch(self.mpm_model.plastic_viscosity)

            def stats(t):
                if t.numel() == 0:
                    return (None, None, None)
                return (t.min().item(), t.mean().item(), t.max().item())

            E_stats = stats(E_t)
            mu_stats = stats(mu_t)
            lam_stats = stats(lam_t)
            yield_stats = stats(yield_t)
            pv_stats = stats(pv_t)
            print("[MPM PARAM STATS] scale_E: ", self.mpm_model.scale_E)
            print("[MPM PARAM STATS] scale_ys: ", self.mpm_model.scale_ys)
            print("[MPM PARAM STATS] scale_pv: ", self.mpm_model.scale_pv)
            print("[MPM PARAM STATS] E: min/mean/max = {:.6g}/{:.6g}/{:.6g}".format(*E_stats))
            print("[MPM PARAM STATS] mu: min/mean/max = {:.6g}/{:.6g}/{:.6g}".format(*mu_stats))
            print("[MPM PARAM STATS] lam: min/mean/max = {:.6g}/{:.6g}/{:.6g}".format(*lam_stats))
            if yield_stats[0] is not None:
                print("[MPM PARAM STATS] yield_stress: min/mean/max = {:.6g}/{:.6g}/{:.6g}".format(*yield_stats))
            if pv_stats[0] is not None:
                print("[MPM PARAM STATS] plastic_viscosity: min/mean/max = {:.6g}/{:.6g}/{:.6g}".format(*pv_stats))
        except Exception as e:
            print("[MPM PARAM STATS] failed to compute stats:", e)
        if "rpic_damping" in kwargs:
            self.mpm_model.rpic_damping = kwargs["rpic_damping"]
        if "softening" in kwargs:
            val = kwargs["softening"]
            wp.launch(
                kernel=set_value_to_float_array,
                dim=self.n_particles,
                inputs=[self.mpm_model.softening, val],
                device=device,
            )
        ...
        def apply_force(
            ...
            if time >= param.start_time and time < param.end_time:
                if param.mask[p] == 1:
                    ...
                    impulse = impulse / state.particle_mass[p]
                    # impulse = impulse / (state.particle_mass[p] * 1e-3 + 1e-8)
    ...
    def release_particles_sequentially_size(
        self, normal, start_position, end_position, num_layers, start_time, end_time, size, point
    ):
        num_layers = num_layers
        _point = [0, 0, 0]
        _size = [0, 0, 0]
        ...
        for i in range(3):
            if normal[i] == 0:
                ...
                axis = i
                _point[i] = start_position

        half_length_portion = wp.abs(start_position - end_position) / num_layers
        end_time_portion = end_time / num_layers
        for i in range(num_layers):
            _size[axis] = half_length_portion * i
            self.enforce_particle_velocity_translation(
                point=_point,
                size=_size,
                velocity=[0, 0, 0],
